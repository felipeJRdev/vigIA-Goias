"""
vigIA — Estágio 1 | Fase 1d: Clima 2026 por município (Open-Meteo)
Entrada:  ../dados/mapeamento_municipio.csv
          ../dados/bdqueimadas_2026-01-01_2026-06-03.csv  (para determinar data_fim)
Saída:    ../dados/clima_2026.csv
"""
...
BDQ          = os.path.join(DADOS, "bdqueimadas_2026-01-01_2026-06-03.csv")
SAIDA        = os.path.join(DADOS, "clima_2026.csv")
LIMIAR_CHUVA = 0.1
MAX_WORKERS  = 3  # archive API tem limite mais restrito que forecast

print("=" * 60)
print("  vigIA E1 — Fase 1d: Clima 2026 (Open-Meteo)")
print("=" * 60)

mapa    = pd.read_csv(MAPA)
df26    = pd.read_csv(BDQ, parse_dates=["DataHora"])
data_fim = str(df26["DataHora"].dt.date.max())
print(f"\n  {len(mapa)} municípios | 2025-12-01 → {data_fim}")
...
def baixar_municipio_2026(nome, lat, lon, retries=5):
...
if len(pendentes) == 0:
    ...
    def _worker(row):
        return row["Municipio"], baixar_municipio_2026(row["Municipio"], row["Latitude"], row["Longitude"])
    ...
    if novos_count:
        print(f"\n  Salvos: {novos_count} municípios em dados/clima_2026.csv")
...
print(f"  Salvo: dados/clima_2026.csv")
print(f"{'='*60}")
print("\n[OK] E1 Fase 1d concluída!")
