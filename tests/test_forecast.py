...
Schema esperado (gerado por `pbl/exportar_json.py`):

    {
      "gerado_em": "2026-06-19T07:57-03:00",
      "dias": ["2026-06-20", ..., "2026-06-24"],
      "municipios": {
        "2026-06-20": [
          {"nome": "...", "lat": ..., "lon": ..., "prob": ..., "pct": ..., "risco": "...", "seco": ...},
          ...
        ], ...
      },
      "celulas": {
        "2026-06-20": [
          {"lat": ..., "lon": ..., "prob": ..., "pct": ..., "risco": "...", "mun": "...", "seco": ...},
          ...
        ], ...
      }
    }
...
class TestEstrutura:
    ...
    def test_campos_obrigatorios_municipios(self, registros_municipios):
        ...
        for dia, rec in registros_municipios:
            for campo in ("nome", "lat", "lon", "prob", "pct", "risco"):
    ...
    def test_campos_obrigatorios_celulas(self, registros_celulas):
        ...
        for dia, rec in registros_celulas:
            for campo in ("lat", "lon", "prob", "pct", "risco", "mun"):
...
class TestValores:
    ...
    def test_percentis_municipios_entre_0_e_1(self, registros_municipios):
        for dia, rec in registros_municipios:
            pct = float(rec["pct"])
            assert 0.0 <= pct <= 1.0, f"[{dia}] pct fora de [0,1] em {rec['nome']}: {pct}"
    ...
    def test_percentis_celulas_entre_0_e_1(self, registros_celulas):
        for dia, rec in registros_celulas:
            pct = float(rec["pct"])
            assert 0.0 <= pct <= 1.0, f"[{dia}] pct de célula fora de [0,1]: {pct}"
...
# --------------------------------------------------------------------------- #
# 3) Coerência percentil x risco
# --------------------------------------------------------------------------- #
class TestCoerencia:
    """
    As classes BAIXO/MÉDIO/ALTO são derivadas do campo `pct` (percentil diário),
    não da probabilidade bruta. Limiares: pct < 0.70 → BAIXO, pct >= 0.90 → ALTO.
    """
    ...
    def _por_classe(self, registros):
        ...
        for _dia, rec in registros:
            if rec["risco"] in faixas:
                faixas[rec["risco"]].append(float(rec["pct"]))
        return faixas

    def test_baixo_abaixo_do_limiar(self, registros_municipios):
        faixas = self._por_classe(registros_municipios)
        for pct in faixas["BAIXO"]:
            assert pct < LIMIAR_BAIXO_MEDIO + EPS, f"BAIXO com percentil alto demais: {pct}"

    def test_alto_acima_do_limiar(self, registros_municipios):
        faixas = self._por_classe(registros_municipios)
        for pct in faixas["ALTO"]:
            assert pct >= LIMIAR_MEDIO_ALTO - EPS, f"ALTO com percentil baixo demais: {pct}"

    def test_medio_dentro_da_faixa(self, registros_municipios):
        faixas = self._por_classe(registros_municipios)
        for pct in faixas["MÉDIO"]:
            assert LIMIAR_BAIXO_MEDIO - EPS <= pct < LIMIAR_MEDIO_ALTO + EPS, (
                f"MÉDIO com percentil fora da faixa esperada [0.70, 0.90): {pct}"
            )
...
class TestFrescor:
    ...
    @pytest.mark.skipif(
        ...
        reason="Checagem de frescor é um monitor de produção; "
               "habilite com VIGIA_CHECK_FRESHNESS=1.",
    )
